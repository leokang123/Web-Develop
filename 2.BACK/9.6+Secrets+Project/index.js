...
app.get("/secrets",ensureAuthenticated, async (req, res) => {
  const user = req.user;
  const result = await db.query("SELECT secret FROM users WHERE email = $1", [user.email]);
...
app.post("/submit", ensureAuthenticated, async (req, res) => {
  ...
  const user = req.user;
  try {
    const result = await db.query("UPDATE users SET secret = $1 WHERE email = $2", [secret, user.email]);
    res.redirect('/secrets');
  } catch (err) {
    console.log(err);
  }
